...
function getPrescriptionNumber(appointmentId: string, date: Date) {
  ...
  return `RX-${datePart}-${shortId}`;
}

async function getAppointmentWithPrescription(appointmentId: string) {
...
function addHeader(doc: jsPDF, prescriptionNumber: string) {
  const pageWidth = doc.internal.pageSize.getWidth();
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  doc.text("Receita Médica", pageWidth - 28, 28, {
    align: "right",
  });

  doc.setFont("helvetica", "normal");
  doc.setFontSize(8);
  doc.text(prescriptionNumber, pageWidth - 28, 36, {
    align: "right",
  });
...
async function createPdfBuffer(
  appointment: NonNullable<
    Awaited<ReturnType<typeof getAppointmentWithPrescription>>
  >
) {
  const generatedAt = new Date();
  const prescriptionNumber = getPrescriptionNumber(
    appointment.id,
    appointment.date
  );
  ...
  doc.setProperties({
    title: `Receita Médica - ${prescriptionNumber}`,
    author: "CannaDoctor",
    creator: "CannaDoctor",
  });

  addHeader(doc, prescriptionNumber);

  doc.setTextColor(8, 85, 63);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(16);
  doc.text("Receita Médica", 105, 58, { align: "center" });

  doc.setTextColor(mutedColor);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(9);
  doc.text(`Número da receita: ${prescriptionNumber}`, 105, 66, {
    align: "center",
  });
  ...
  currentY = addSection(
    doc,
    "Quantidade",
    appointment.prescription?.quantity,
    currentY
  );
...
export async function GET(_request: Request, { params }: RouteProps) {
  ...
  const appointment = await getAppointmentWithPrescription(appointmentId);
  ...
  return new Response(new Uint8Array(pdfBuffer), {
    ...
    headers: {
      ...
      "Content-Disposition": `inline; filename="${prescriptionNumber}.pdf"`,
    },
  });
}