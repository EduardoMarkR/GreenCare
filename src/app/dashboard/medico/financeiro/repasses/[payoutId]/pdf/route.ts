...
export async function GET(_: Request, { params }: Props) {
  ...
  if (!userId || activeProfile !== "DOCTOR") {
  ...
  doc.setFontSize(10);
  doc.text("Comprovante de repasse medico", 14, 24);
  ...
  doc.setFontSize(15);
  doc.text("Resumo do repasse", 14, 48);
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      [
        "Periodo",
        `${formatDate(payout.startDate)} ate ${formatDate(payout.endDate)}`,
      ],
  ...
  doc.setTextColor(8, 85, 63);
  doc.setFontSize(15);
  doc.text("Pagamentos vinculados", 14, summaryFinalY + 14);
